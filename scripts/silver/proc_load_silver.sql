...
BEGIN
	...
	SET @batch_start_time = GETDATE();
		...
		PRINT '----------------------------------------------------';
		-- Cleaned up silver.crm_cust_info
		...
		)t WHERE flag_last = 1 AND cst_key != 'PO25'
		SET @end_time = GETDATE();
		PRINT '>> Load Duration: ' + CAST(DATEDIFF(second, @start_time, @end_time) AS NVARCHAR) + ' seconds';
		PRINT '>> -------------';
		-- Cleaned up silver.crm_prd_info
		SET @start_time = GETDATE();
		...
		PRINT '>> Load Duration: ' + CAST(DATEDIFF(second, @start_time, @end_time) AS NVARCHAR) + ' seconds';
		PRINT '>> -------------';
		-- Cleaned up silver.crm_sales_details
		SET @start_time = GETDATE();
		...
		SELECT
			...
			END AS sls_order_dt,
			CASE WHEN sls_order_ship_dt = 0 OR LEN(sls_order_dt) != 8 THEN NULL
				ELSE CAST(CAST(sls_order_ship_dt AS VARCHAR) AS DATE)
			END AS sls_order_ship_dt,
		...
		PRINT '>> Load Duration: ' + CAST(DATEDIFF(second, @start_time, @end_time) AS NVARCHAR) + ' seconds';
		PRINT '>> -------------';
		-- Cleaned up silver.erp_cust_az12
		PRINT '----------------------------------------------------';
		...
		PRINT '>> Load Duration: ' + CAST(DATEDIFF(second, @start_time, @end_time) AS NVARCHAR) + ' seconds';
		PRINT '>> -------------';
		-- Cleaned up silver.erp_loc_a101
		SET @start_time = GETDATE();
		...
		PRINT '>> Load Duration: ' + CAST(DATEDIFF(second, @start_time, @end_time) AS NVARCHAR) + ' seconds';
		PRINT '>> -------------';
		-- Cleaned up silver.erp_px_cat_g1v2
